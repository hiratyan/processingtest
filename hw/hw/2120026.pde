String screen = "start"; //ゲーム画面を指定する変数．
//"start" はスタート画面，"game" はゲーム画面．"end" は終了画面．
...
void mousePressed(){
    if(screen == "start"){
        ...
        screen = "game";
    } else if(screen == "game"){//ゲーム中
        if(mouseButton == LEFT) {//左クリックによってx軸の速度が反転．
            pinball[0].vx *= -1;
            pinball[0].vy *= 1;            
        }
        if(mouseButton == RIGHT){//右クリックによってy軸の速度が反転．
            pinball[0].vx *= 1;
            pinball[0].vy *= -1;
        }
